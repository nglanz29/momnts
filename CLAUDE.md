...
**The entire app is ONE self-contained HTML file: `momnts.html`** (~4,600 lines). HTML + CSS + JS all inline, one `<script>` block. No build step, no bundler, no framework, no external JS/CSS files, no npm dependencies. The only external runtime dependencies are:
...
1. **Parse check is authoritative.** Extract the inline `<script>` block and confirm it parses (e.g. `node -e "new Function(require('fs').readFileSync('momnts.html','utf8').match(/<script>(?![^>]*src)([\s\S]*?)<\/script>/g).sort((a,b)=>b.length-a.length)[0].replace(/<\/?script[^>]*>/g,''))"`). A syntax error anywhere kills the whole app ("nothing is clickable").
